...
class GetStudyLibDebtsUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Resource<List<String>>> = flow {
        try {
            ...
            if (cookie == null) {
                emit(Resource.Error<List<String>>("LessCookie"))
            }
            if (cookie != null) {
                val data = api_repository.getStudyLibDebts(cookie)
                emit(Resource.Success<List<String>>(data))
            }
            ...
            if (loginAndPassword?.login == null || loginAndPassword.password == null) {
                ...
                try {
                    ...
                    emit(
                        Resource.Error<List<String>>(e.localizedMessage ?: "ConnectionError")
                    )
                } catch (e: IOException) {
                    emit(
